...
void KeyPad_getPressedKey(uint8_t* key)
{
	...
	for(col = 0 ; col < N_col ; col++)
	{
		 ...
		 */
		KEYPAD_PORT_DIR = /*(KEYPAD_PORT_DIR & 0x03) | (0b00100000 << col)*/0xFF;

		/*
		 * clear the output pin column in this trace and enable the internal
		 * pull up resistors for the rows pins
		 */
		KEYPAD_PORT_OUT = (KEYPAD_PORT_OUT & 0x03) | (~(0b00100000 << col) & ~(0x03));

		/* loop for rows */
		for(row = 0 ; row < N_row ; row++)
		{
			/* if the switch is press in this row */
			if(GetBit(KEYPAD_PORT_IN, (row + 2)))
			{
#if (N_col == 3)
				*key = KeyPad_4x3_adjustKeyNumber((row * N_col) + col + 1);
#elif (N_col == 4)
				*key = KeyPad_4x4_adjustKeyNumber((row * N_col) + col + 1);
#endif
			}
		}
	}
