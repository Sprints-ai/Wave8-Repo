...
#include "../../Services/includes/SOS_PCfg.h"
#include "../../MCAL/includes/DIO.h"

/********Internal Structures************/
/*
typedef struct strSOS_TaskInternalInfo_t{
	uint32_t DelayCounter;
	uint16_t TaskId;
}strSOS_TaskInternalInfo_t;*/

/*******Static Global Variables*********/
static strSOS_TASK_Cfg_t *gstr_TasksBuffer[SOS_TASK_BUFFER_SIZE];
/*
static strSOS_TaskInternalInfo_t *gstr_TaskPrivateBuffer[SOS_TASK_BUFFER_SIZE];
static strSOS_TaskInternalInfo_t  gstr_TaskPrivate[SOS_TASK_BUFFER_SIZE];*/
...
ERROR_STATUS SOS_Init(void){
	...
	if(gu8_InitFlag == FALSE){
		...
		if(aES_errorStatus == E_OK){
			...
			astr_TimerCfg.Timer_Prescaler = SOS_RES_PRESCALE;
...
*
*/
ERROR_STATUS SOS_Create_Task(strSOS_TASK_Cfg_t* task){
	...
	if(gu8_InitFlag == TRUE){
		...
		if(task != NULL){
			...
			if( (task->Delay_TimeMs%SOS_RES_TICK) != ZERO)
			{
				aES_errorStatus = E_INVALID_PARAMETER;
			}
			if (aES_errorStatus == E_OK)
			{
				task->Delay_Counter = task->Delay_TimeMs/SOS_RES_TICK;
	...
	if(gu8_ISR_Flag == TRUE){
		...
		while(gstr_TasksBuffer[au16_TaskIterator] != NULL){
			...
			if (gstr_TasksBuffer[au16_TaskIterator]->Delay_Counter == 0)
			{
				...
				{
					gstr_TasksBuffer[au16_TaskIterator]->Delay_Counter = gstr_TasksBuffer[au16_TaskIterator]->Delay_TimeMs/SOS_RES_TICK;
				}
